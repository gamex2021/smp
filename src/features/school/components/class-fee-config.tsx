...
export function ClassFeeConfig({ schoolId }: ClassFeeConfigProps) {
  ...
  //   *Get the fee structure for the selected class and term
  ...
  >(
    feeStructure?.fees.map((fee) => ({
      ...fee,
      installmentConfig: fee.installmentConfig
        ? {
            ...fee.installmentConfig,
            maximumInstallments: fee.installmentConfig.maximumInstallments ?? 2,
            minimumFirstPayment: fee.installmentConfig.minimumFirstPayment ?? 0,
            installmentDueDates:
              fee.installmentConfig.installmentDueDates ?? [],
          }
        : undefined,
    })) ?? [],
  );
  ...
  console.log("this is the fee structure", feeStructure);
  console.log("this is the fees", fees);
  ...
  if (!currentYear) {
    return (
      <Alert>
        <AlertDescription>
          Please configure an academic year first
        </AlertDescription>
      </Alert>
    );
  }
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setSelectedTerm(Number.parseInt(value))
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-4">
            {fees.map((fee) => (
              <div key={fee.id} className="space-y-4 rounded-lg border p-4">
                          ...
                          newFees.map((f) => ({
                            ...
                            installmentConfig: f.installmentConfig
                              ? {
                                  ...f.installmentConfig,
                                  maximumInstallments:
                                    f.installmentConfig.maximumInstallments ??
                                    2,
                                }
                              : undefined,
                          })),
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-gray-500">
                      Mark this fee as required for all students
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      const newFees = fees.map((f) =>
                        f.id === fee.id ? { ...f, isCompulsory: checked } : f,
                      );
                      setFees(newFees);
                    }}
                  />
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="grid gap-2">
                        ...
                        <Input
                          ...
                          max="100"
                          value={
                            fee.installmentConfig?.minimumFirstPayment ?? 0
                          }
                          onChange={(e) => {
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input
                          ...
                          max="12"
                          value={
                            fee.installmentConfig?.maximumInstallments ?? 2
                          }
                          onChange={(e) => {
                            const newFees: any = fees.map((f) =>
                              f.id === fee.id
