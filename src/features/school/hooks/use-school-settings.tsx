...
export function useSchoolSettings(schoolId: Id<"schools">) {
  ...
  const academicConfig = useQuery(api.queries.school.getAcademicConfig, {
    schoolId,
  });
  ...
  const configureAcademicYear = useMutation(
    api.mutations.school.configureAcademicYear,
  );
  const configureTerm = useMutation(api.mutations.school.configureTerm);
  const updateTermSettings = useMutation(
    api.mutations.school.updateTermSettings,
  );
  ...
  const handleTermConfig = async (
    academicConfigId: Id<"academicConfig">,
    data: {
      name: string;
      startDate: string;
      endDate: string;
