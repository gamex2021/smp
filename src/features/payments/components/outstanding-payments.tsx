...
export function OutstandingPayments({ schoolId }: { schoolId: Id<"schools"> }) {
    ...
    const outstandingPayments = useQuery(api.queries.fees.getOutstandingPayments, {
        schoolId,
        classId: filters.classId !== "all" ? filters.classId : undefined,
        academicYear: filters.academicYear || undefined,
        termId: filters.termId !== "all" ? Number.parseInt(filters.termId) : undefined,
        isCompulsory: true,
    })

    const sendReminder = useAction(api.actions.notifications.sendPaymentReminder)
    const sendBulkReminders = useAction(api.actions.notifications.sendBulkPaymentReminders)

    const handleSendReminder = async (studentId: Id<"users">, outstandingId: Id<"outstandingPayments">) => {
        try {
            await sendReminder({
                studentId,
    ...
    const handleSendBulkReminders = async () => {
        ...
        try {
            ...
            toast.success(`Payment reminders sent to ${selectedStudents.length} students`)
            setSelectedStudents([])
        } catch (error) {
                ...
                <div className="space-y-6">
                    <div className="flex flex-wrap gap-4">
                        <Select
                            ...
                            onValueChange={(value: Id<"classes">) => setFilters((prev) => ({ ...prev, classId: value }))}
                        >
                        ...
                        <Select
                            ...
                            onValueChange={(value) => setFilters((prev) => ({ ...prev, termId: value }))}
                        >
                            <SelectTrigger className="w-[200px]">
                        ...
                        <div className="space-y-6">
                            <div className="rounded-md border">
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead className="w-12">
                                                <Checkbox
                                                    checked={selectedStudents.length === Object.keys(groupedPayments).length}
                                    ...
                                    <TableBody>
                                        {Object.entries(groupedPayments).map(([studentId, data]) => (
                                            <TableRow key={studentId}>
                                                <TableCell>
                                                    <Checkbox
                                                        checked={selectedStudents.includes(studentId as Id<"users">)}
                                                        onCheckedChange={(checked) => {
                                                            setSelectedStudents((prev) =>
                                                                checked ? [...prev, studentId as Id<"users">] : prev.filter((id) => id !== studentId),
                                                            )
                                                        }}
                                                    />
                                                </TableCell>
                                                    ...
                                                    <div className="space-y-1">
                                                        {data.payments.map((payment: any) => (
                                                            <div key={payment._id} className="flex items-center gap-2">
                                                                <span className="text-sm">{payment.feeName}</span>
                                                                <Badge
                                                                    variant={
                                                ...
                                                <TableCell>₦{formatCurrency(data.totalOutstanding)}</TableCell>
                                                <TableCell>
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        onClick={() => {
                                                            ...
                                                            data.payments.forEach(async (payment: any) => {
                                                                await handleSendReminder(studentId as Id<"users">, payment._id)
                                                            })
                                                        }}
                                                    >
                            ...
                            {outstandingPayments?.some((p) => p.status === "overdue") && (
                                <Alert variant="destructive">
                                    ...
                                    <AlertDescription>Some payments are overdue. Please take immediate action.</AlertDescription>
                                </Alert>
                            )}
                        </div>
                    ) : (
                        <div className="text-center py-6 text-gray-500">No outstanding payments found</div>
                    )}
                </div>
            </CardContent>
        </Card>
